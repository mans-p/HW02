...
```
Like the error message says, + should be used to add layers to a plot instead of >%>, since the output from the original ggplot2 call isn't being piped to geom_point(). 
...
```
aes() is for values that are being plotted. Color is a geom_point() option outside of aes().
...
```
alpha determines transparency of the points. Size can be modified with its own argument, size.

```{r}
mpg %>% 
ggplot() + 
  geom_point(mapping = aes(x = displ, y = hwy, color = class), shape=16, size=2) + 
  theme(legend.direction = "horizontal") + 
  theme(legend.position = c(5, 40))

```
Setting the legend position with a vector requires an ordered pair of values between 0 and 1 - not those that correspond to the axes on the plot.
...
geom_smooth() inherits aesthetics from ggplot call. Setting the color in geom_point causes geom_smooth to ignore the colors of the points.
...
```
Make all one color by setting color and fill arguments to the same thing.
Angle of x-axis labels are with respect to the center of the axis label. Changing the hjust argument adjusts the point around which the labels are justified.
